...
def get_top_heroes_graphs(league=None, dates=[None]):
    clauses, params = handle_filters(league=league, dates=dates)
    query = 'SELECT COUNT(*) FROM match_details md' + clauses
    ...
    for key, query_filter in [('picked', pick_filter), ('banned', ban_filter)]:
        query = f'''
            ...
            LIMIT 5;
        '''
        rows = db.query_select(query, params=params)
        results[key].extend([row for row in rows])   # (count, display_name, npc_name)
    most_picked = pd.DataFrame(results['picked'], columns=['picks', 'hero']).sort_values(by='picks')
    most_banned = pd.DataFrame(results['banned'], columns=['bans', 'hero']).sort_values(by='bans')
    ...
    return winrate_fig, picked_fig, banned_fig